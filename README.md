...
- From the menu-bar item, choose **Slack becomes active** to fire the reminder.
...
Voice transcription, LLM parsing, app activity, and browser URL events are intentionally mocked. The saved model still separates the raw request, inferred semantic context, runtime identifiers, confidence, and time fallback so those integrations can replace the mocks without redesigning the UI.
